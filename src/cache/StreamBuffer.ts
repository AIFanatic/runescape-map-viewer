...
export class StreamBuffer {
    ...
    public static create(size: number = 5000): StreamBuffer {
    ...
    public writeBytes(fromBuffer: StreamBuffer | Buffer): void {
    ...
    public read24BitInt(): number {
        this.readerIndex += 3;
        return (
            ((this.buffer[this.readerIndex - 3] & 255) << 16) +
            ((this.buffer[this.readerIndex - 2] & 255) << 8) +
            (this.buffer[this.readerIndex - 1] & 255)
        );
    }
    ...
    public readSignedSmart(): number {
        ...
        if(peek < 128) {
            ...
            return this.readUnsignedShortBE() - 49152;
        }
    }
    ...
    public getSlice(position: number, length: number): StreamBuffer {
        return new StreamBuffer(this.buffer.slice(position, position + length));
    }

    public flip(): StreamBuffer {
