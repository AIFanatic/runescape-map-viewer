...
export class FloorDefinition {
    ...
    public static load(definitionArchive: CacheArchive) {
        const buffer: StreamBuffer = definitionArchive.getFileData("flo.dat");
        FloorDefinition.count = buffer.readUnsignedShortBE();
        if (FloorDefinition.cache == null) {
            FloorDefinition.cache = new Array(FloorDefinition.count).fill(null);
        }
    ...
    public loadDefinition(buffer: StreamBuffer) {
        while ((true)) {{
            const attributeId: number = buffer.readUnsignedByte();
            ...
            case 7:
                ...
                this.shiftRGBColors(buffer.read24BitInt());
                this.hue2 = oldHue2;
